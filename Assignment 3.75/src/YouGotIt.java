...
public class YouGotIt {
	...
	//initial setup
		...
		int guess = 0; //creates variable for user guess
		
	//rules
		JOptionPane.showMessageDialog(null, "The computer chooses a number between 1 and 300, you then have 10 chances to guess the computer's number.\nIf you guess correctly in time, you win! If you guess more than 10 times... you'll lose.", "Rules", JOptionPane.INFORMATION_MESSAGE);
		
	//main control flow
		while ((num != guess) && (counter < 10)) { //loop continues so long as user guesses incorrectly or guess count goes too high
			String guessPromt = JOptionPane.showInputDialog(null, "Guess a number between 1 and 300.", "Guess", JOptionPane.QUESTION_MESSAGE); //prompts user for a guess
			...
			if (num == guess) { //checks to see if user guessed correctly
				...
				JOptionPane.showMessageDialog(null, "Error: Input out of range. Guess again.", "Error", JOptionPane.ERROR_MESSAGE);
